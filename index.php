<?php

include ("authentication/dbconnection.php");

session_start();

if (isset($_SESSION['user_id'])) {
    $user_id = $_SESSION['user_id'];
} else {
    $user_id = '';
}
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>The Perficient</title>
    <link rel="stylesheet" href="components/css/style.css">

    <link rel="stylesheet" href="https://unpkg.com/swiper@8/swiper-bundle.min.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/typed.js/2.0.12/typed.min.js"></script>
    <style>
        .faq-container {
            max-width: 800px;
            margin: 20px auto;
            padding: 20px;
            background-color: #fff;
            border-radius: 5px;
            box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);
        }

        .faq-item {
            margin-bottom: 20px;
        }

        .question {
            cursor: pointer;
            font-weight: bold;
            color: #497649;
            border-bottom: 1px solid #ccc;
            padding: 10px;
            display: flex;
            align-items: center;
            justify-content: space-between;
        }

        .answer {
            padding: 10px;
            display: none;
            color: gray;
        }
        .typedText {
            font-size: 20px;
            font-family: Arial, sans-serif;
            color:#c79ff9;
            border-right: 2px solid #333; /* creates a cursor effect */
            padding-right: 5px; /* space for cursor effect */
        }
    </style>

</head>

<body>
    <div class="container">
        <div class="nav-main">
            <nav class="navbar">
                <div class="logo">
                    <a href="#" class="logo-text">The Perficient</a>
                </div>
                <div class="nav-items">
                    <a href="index.php" class="nav-item">HOME</a>
                    <a href="aboutus.php" class="nav-item">ABOUT</a>
                    <a href="services.php" class="nav-item">SERVICE</a>
                    <a href="events.php" class="nav-item">EVENTS</a>
                    <a href="contact.php" class="nav-item">CONTACT</a>
                </div>

                <div class="icons">
                    <?php
                    if (isset($_SESSION['user_id'])) {
                        echo '<div class="profile-dropdown">';
                        echo '<select onchange="profileMenu(this.value)">';
                        echo '<option value="">Profile</option>';
                        echo '<option value="dashboard">Dashboard</option>';
                        echo '<option value="edit_profile">Edit Profile</option>';
                        echo '<option value="logout">Logout</option>';
                        echo '</select>';
                        echo '</div>';
                    } else {
                        echo '<button class="book-btn" onclick="registerbtn()">Register</button>';
                        echo '<button class="book-btn" onclick="loginbtn()">Login</button>';
                    }
                    ?>

                    <script>
                        function registerbtn() {
                            window.location.href = "authentication/createaccount.php";
                        }
                        function loginbtn() {
                            window.location.href = "authentication/login.php";
                        }
                        function profileMenu(option) {
                            if (option === "dashboard") {
                                window.location.href = "dashboard1.php";
                            } else if (option === "edit_profile") {
                                window.location.href = "update_profile.php";
                            } else if (option === "logout") {
                                window.location.href = "authentication/logout.php";
                            }
                        }
                    </script>
                </div>

            </nav>


            <!-- <div id="user-btn" class="fas fa-user"></div> -->

            <div class="box">
            </div>
            <div class="nav-content">
                <div class="left-side">
                    <div class="first-content">
                        <span style="font-size: 35px;">Perficient = <span class="typedText"></span></span>
                    </div>
                    <script>

                        let typingEffect = new Typed(".typedText", {
                            strings: ["Proficient", "Perfect", "Efficient"],
                            loop: true,
                            typeSpeed: 50,
                            backSpeed: 60,
                            backDelay: 1000
                            
                        })

                    </script>

                    <div class="nav-con-about">
                        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Neque debitis quo excepturi eos
                            nisi impedit modi omnis, alias qui suscipit?</p>
                            
                    </div>
                    <div class="book-tbl-btn">
                        <button class="book-btn" onclick="services()">Create event</button>
                        <script>
                            function services() {
                                window.location.href = "services.html";
                            }
                        </script>
                    </div>
                </div>
            </div>
        </div>

        <main class="main-class">


            <div class="services">
                <h2>OUR SERVICES</h2>
                <div class="overview-grid">
                    <div class="box1 box">
                        <span>
                            <img src="https://cdn.shopify.com/s/files/1/2196/3271/files/Indian_wedding_planner_vs_Indian_wedding_stylist_1024x1024.jpg?v=1679913759"
                                alt="" style=" width: 295px;">
                        </span>
                        <span class="bold">Wedding Planning</span>
                        <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. At, saepe.</span>
                    </div>
                    <div class="box1 box">
                        <span>
                            <img src="https://www.oyorooms.com/blog/wp-content/uploads/2018/02/How-much-space-does-it-has.png"
                                alt="" style="width: 295px;">
                        </span>
                        <span class="bold"><a class="bold" href="qualityfood.html">Birthday Parties</a></span>
                        <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. At, saepe.</span>
                    </div>
                    <div class="box1 box">
                        <span>
                            <img src="data:image/jpeg;base64,"
                                alt="" style="width: 295px;">
                        </span>
                        <span class="bold">Private Parties</span>
                        <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. At, saepe.</span>
                    </div>
                    <div class="box1 box">
                        <span>
                            <img src="https://www.rajwadaevents.com/uploaded-files/feature-images/Corporate-Events03_50_48.jpg"
                                alt="" style="width: 295px;">
                        </span>
                        <span class="bold">Corporate Events</span>
                        <span>Lorem ipsum dolor sit amet consectetur adipisicing elit. At, saepe.</span>
                    </div>

                </div>
            </div>

            <br>
            <h2 class="txt">GALLERY</h2>
            <br>
            <div class="gallery " style="height: 900px; width: 900px;">
                <div class="container1">

                    <!-- Full-width images with number text -->
                    <div class="mySlides">
                        <div class="numbertext">1 / 6</div>
                        <img src="https://manhattanbride.com/wp-content/uploads/2019/04/Indian-Traditions39-Ceremony-506-Christopher-Brock-Photography.jpg"
                            style="width:100%; border-radius: 20px;">
                    </div>

                    <div class="mySlides">
                        <div class="numbertext">2 / 6</div>
                        <img src="https://img.staticmb.com/mbcontent/images/crop/uploads/2023/8/Heaven_4_0_1200.jpg "
                            style="width:100%;  border-radius: 20px;">
                    </div>

                    <div class="mySlides">
                        <div class="numbertext">3 / 6</div>
                        <img src="https://www.aurusjewels.com/cdn/shop/files/Indian_wedding_rituals.jpg?v=1684403751"
                            style="width:100%;  border-radius: 20px;">
                    </div>

                    <div class="mySlides">
                        <div class="numbertext">4 / 6</div>
                        <img src="https://c.files.bbci.co.uk/69CF/production/_115378072_image00008.jpg"
                            style="width:100%;  border-radius: 20px;">
                    </div>

                    <div class="mySlides">
                        <div class="numbertext">5 / 6</div>
                        <img src="https://assets.thehansindia.com/h-upload/2019/12/12/244550-indian-wedding.webp"
                            style="width:100%;  border-radius: 20px;">
                    </div>

                    <div class="mySlides">
                        <div class="numbertext">6 / 6</div>
                        <img src="https://im.idiva.com/content/2023/Aug/thumbnail-2_64ddecbbb0f1a.jpg"
                            style="width:100%" style="width:100%;  border-radius: 20px;">
                    </div>

                    <!-- Next and previous buttons -->
                    <a class="prev" onclick="plusSlides(-1)">&#10094;</a>
                    <a class="next" onclick="plusSlides(1)">&#10095;</a>

                    <!-- Image text -->

                    <!-- Thumbnail images -->

                </div>
                <h2 style="margin-top: 50px;" class="txt">Testimonials</h2>
            </div>

            <div class="testimonial-container">

                <div class="testimonial-box">
                    <p>"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut
                        labore et dolore magna aliqua."</p>
                    <p class="author">- John Doe</p>
                </div>
                <div class="testimonial-box">
                    <p>"Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo
                        consequat."</p>
                    <p class="author">- Jane Smith</p>
                </div>
                <div class="testimonial-box">
                    <p>"Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla
                        pariatur."</p>
                    <p class="author">- Emily Johnson</p>
                </div>
                <div class="testimonial-box">
                    <p>"Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim
                        id est laborum."</p>
                    <p class="author">- Michael Brown</p>
                </div>
            </div>


            <div class="faq-container">
                <div class="faq-item">
                    <h2 style="color:#35763e;">Faqs</h2>
                    <div class="question">How to register? <img src="sign.svg" alt=""></div>
                    <div class="answer">Simply click on register button that located on top-right at navigation bar then
                        fill all provided details after that you got a mail that has your details to login on this web
                        application </div>
                </div>
                <div class="faq-item">
                    <div class="question">How to register on events ? <img src="sign.svg" alt=""></div>
                    <div class="answer">It is a long established fact that a reader will be distracted by the readable
                        content of a
                        page when looking at its layout.</div>
                </div>
                <div class="faq-item">
                    <div class="question">How to contact our team? <img src="sign.svg" alt=""></div>
                    <div class="answer">Contrary to popular belief, Lorem Ipsum is not simply random text.</div>
                </div>
            </div>

            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    const questions = document.querySelectorAll('.question');
                    questions.forEach(function (question) {
                        question.addEventListener('click', function () {
                            const answer = this.nextElementSibling;
                            if (answer.style.display === 'block') {
                                answer.style.display = 'none';
                            } else {
                                answer.style.display = 'block';
                            }
                        });
                    });
                });
            </script>

        </main>
        <div class="foot">
            <footer>
                <p>&copy; Copyrights reserved 2024 | <a href="http://www.perficient.com" target="_blank"
                        style="text-decoration: none; color: #ffffff;">www.perficient.com</a></p>

            </footer>
        </div>
    </div>

</body>

</html>
<script>
    let slideIndex = 1;
    showSlides(slideIndex);

    // Next/previous controls
    function plusSlides(n) {
        showSlides(slideIndex += n);
    }

    // Thumbnail image controls
    function currentSlide(n) {
        showSlides(slideIndex = n);
    }

    function showSlides(n) {
        let i;
        let slides = document.getElementsByClassName("mySlides");
        let dots = document.getElementsByClassName("demo");
        let captionText = document.getElementById("caption");
        if (n > slides.length) { slideIndex = 1 }
        if (n < 1) { slideIndex = slides.length }
        for (i = 0; i < slides.length; i++) {
            slides[i].style.display = "none";
        }
        for (i = 0; i < dots.length; i++) {
            dots[i].className = dots[i].className.replace(" active", "");
        }
        slides[slideIndex - 1].style.display = "block";
        dots[slideIndex - 1].className += " active";
        captionText.innerHTML = dots[slideIndex - 1].alt;
    }

</script>